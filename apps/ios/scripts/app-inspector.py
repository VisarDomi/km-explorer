#!/usr/bin/env python3
"""Inspect only the installed bundled Ytb app. Use the Mac inspector-venv."""
...
SNAPSHOT = """JSON.stringify({url:location.href,visible:document.visibilityState,scrollY,cards:document.querySelectorAll('.ke-card').length,loadedImages:[...document.images].filter(i=>i.naturalWidth>0).length,backupPrompt:!!document.querySelector('#reader-backup-setup'),copyVisible:!!document.querySelector('.ke-video-copy:not([hidden])'),video:[...document.querySelectorAll('video')].map(v=>({ready:v.readyState,error:v.error?.code,time:v.currentTime,paused:v.paused,muted:v.muted,inline:v.playsInline,nativeControls:v.controls})),scripts:[...document.scripts].map(s=>s.src)})"""
async def main():
    ...
    try:
        ...
        candidates=[p for p in pages if p.application.bundle in args.host_bundle and p.page.web_url.startswith('ytb://app/')]
        print('PAGES',json.dumps([{'id':p.page.id_,'url':p.page.web_url} for p in candidates]),flush=True)
        if len(candidates)!=1: raise RuntimeError('Open/unlock Ytb; expected one bundled reader page')
        pair=candidates[0];session=await asyncio.wait_for(inspector.inspector_session(pair.application,pair.page),15)
        session.response_methods['Console.messageAdded']=lambda e:print('CONSOLE',json.dumps({k:e['params'].get('message',{}).get(k) for k in ['level','text','parameters']}),flush=True)
